...
// flash_ctrl_host_random_read Test
...
class flash_ctrl_basic_rw_vseq extends flash_ctrl_base_vseq;
  ...
  constraint flash_op_c {
    ...
    flash_op.op inside {flash_ctrl_pkg::FlashOpProgram, flash_ctrl_pkg::FlashOpRead};
  ...
  virtual task body();
    ...
    for (int k = 0; k < num_trans; k++) begin
      ...
      flash_ctrl_base_vseq::flash_ctrl_write_extra(flash_op, flash_op_data, 1, mubi4_test_true_strict(scramble_en), mubi4_test_true_strict(ecc_en));
      ...
      for (int i = 0; i < flash_op.num_words; i++) begin
        ...
        do_direct_read(.addr(read_addr), .mask('1), .blocking(cfg.block_host_rd), .check_rdata(1),
                       .instr_type(instr_type), .rdata(rdata), .exp_rdata(flash_op_data[i]), .exp_err_rsp(exp_err),
                       .completed(comp));
      ...
      for (int i = 0; i < flash_op.num_words; i++) begin
        if (flash_op_data[i] !== flash_op_rdata[i])
          `uvm_error("rd-err", $sformatf("Read-back error: exp: %x is: %x", flash_op_data[i], flash_op_rdata[i]))
      csr_utils_pkg::wait_no_outstanding_access();
      end
    end
